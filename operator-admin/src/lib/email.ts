...
/**
 * Sends a self-service password reset email to an operator (/forgot-password)
 * or a consumer (/account/forgot-password) who requested it — shared by both,
 * hence the account-type-agnostic copy ("your Happy Hour Compass account").
 *
 ...
 *
 * `resetLink` is built from a Supabase recovery link generated by
 * auth.admin.generateLink({ type: 'recovery', ... }) — either the raw
 * action_link (operator) or a token_hash-shaped link pointing at our own
 * domain (consumer — see requestConsumerPasswordReset for why).
 */
export async function sendPasswordResetEmail({
  ...
  const html = emailLayout(`
          ...
          <p style="margin:0 0 16px;font-size:15px;color:#475569;line-height:1.6;">${greeting}</p>
          <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.6;">
            You requested a password reset for your Happy Hour Compass account.
