...
function NoteEntry({ note }: { note: SubmissionNote }) {
...
export default function InternalNotesSection({
  ...
  /** Legacy review_notes value from the submission row — shown read-only at bottom. */
  ...
  const boundAction = (addSubmissionNoteAction as any).bind(null, submissionId);
  const [state, formAction, pending] = useActionState<AddNoteState, FormData>(
    boundAction,
    INITIAL_STATE
  );

  // Refresh server data so newly-added note appears in the list,
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      ...
      {hasNotes ? (
        <div className="-mb-1">
          {/* New-style notes — newest first (order comes from the query) */}
          {initialNotes.map((n) => (
            <NoteEntry key={n.id} note={n} />
          ))}

          {/* Legacy review_notes displayed as a read-only historical entry */}
          {legacyNote && (
            <div className="py-3 border-b border-gray-100 last:border-0">
              <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed mb-1.5">
                {legacyNote}
              ...
              <p className="text-[11px] text-gray-400 italic">
                Legacy note (migrated from review_notes)
              </p>
            </div>
          )}
        </div>
