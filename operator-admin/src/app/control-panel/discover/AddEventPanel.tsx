...
import { addEventToRailAction } from "./actions";
...
export function AddEventPanel({ candidates, railKey }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState<CPFeaturedEventItem | null>(null);
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      const result = await addEventToRailAction(railKey, selected.eventUuid, null, fd);
      if (!result.success) {
  ...
  if (!open) {
    return (
      ...
      >
        <span className="text-base leading-none">+</span> Add event to this rail
      </button>
    );
  }
  ...
  return (
    <div className="mt-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
      <p className="text-xs font-medium text-amber-800 mb-3">
        Add an event to this rail. Search by event name or venue name.
      </p>
      ...
      {!selected && (
        <div className="mb-3">
              ...
              {filtered.map((e) => (
                <li key={e.eventUuid}>
                  <button
                    ...
                    className="w-full flex items-start justify-between px-3 py-2 text-left text-sm hover:bg-amber-50 transition-colors"
                  >
                    <div className="min-w-0 flex-1">
                      <span className="font-medium text-slate-800 block truncate">{e.title}</span>
                      <span className="text-xs text-gray-400 block truncate">
                        {e.venueName} · {e.nextOccurrenceLabel}
                      </span>
                    </div>
                    <span
                      className={`ml-2 text-xs font-medium px-2 py-0.5 rounded-full capitalize shrink-0 mt-0.5 ${
                        PLAN_BADGE[e.operatorPlan] ?? PLAN_BADGE.free
                      }`}
                    >
                      {e.operatorPlan}
                    </span>
          ...
          {query.trim().length >= 1 && filtered.length === 0 && (
            <p className="mt-2 text-xs text-gray-400">No matching events found.</p>
          )}
        </div>
      )}
      ...
      {selected && (
        <div className="mb-3">
          <div className="flex items-start justify-between p-2 bg-white border border-gray-200 rounded-lg">
            <div className="min-w-0 flex-1">
              <span className="text-sm font-medium text-slate-800 block truncate">{selected.title}</span>
              <span className="text-xs text-gray-400 truncate block">
                {selected.venueName} · {selected.nextOccurrenceLabel}
              </span>
            </div>
