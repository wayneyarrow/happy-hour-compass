...
import { restoreEventToAlgorithmAction } from "./actions";
...
export function RemovedEventRow({ event, railKey, reasonType, note, removedBy }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const handleRestore = () => {
    startTransition(async () => {
      await restoreEventToAlgorithmAction(railKey, event.eventUuid);
      router.refresh();
    });
  };
  ...
  return (
    <div
      className={`flex flex-wrap items-start gap-x-4 gap-y-1 px-4 py-3 border-b border-red-50 last:border-0 text-sm ${
        isPending ? "opacity-60" : ""
      }`}
    >
      <div className="flex-1 min-w-0">
        <span className="font-medium text-slate-700 truncate block">{event.title}</span>
        <span className="text-xs text-gray-400 truncate block">{event.venueName}</span>
        {(reasonLabel(reasonType) || note || removedBy) && (
