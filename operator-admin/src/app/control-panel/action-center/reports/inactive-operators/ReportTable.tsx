...
import type { InactiveOperatorsRow } from "@/lib/data/actionCenter";

type SortCol =
  | "name" | "city" | "plan" | "setupHealthScorePct"
  | "daysSinceLastLogin" | "venueViews30d" | "eventViews30d" | "isPublished";

const PAGE_SIZE    = 25;
const DEFAULT_SORT: SortCol = "daysSinceLastLogin";
...
export default function InactiveOperatorsTable({ rows }: { rows: InactiveOperatorsRow[] }) {
  ...
  const handleExport = () => {
    const headers = ["Venue", "City", "Plan", "Health Score %", "Last Login", "Days Since Last Login", "Venue Views (30d)", "Event Views (30d)", "Published"];
    ...
    downloadCsv(`inactive-operators-${new Date().toISOString().slice(0, 10)}.csv`, buildCsv(headers, csvRows));
  };
                    ...
                    <tr key={r.id} onClick={() => router.push(`/control-panel/venues/${r.id}`)}
                      ...
                      <td className="px-4 py-3 text-gray-500 whitespace-nowrap text-xs">
                        {r.operatorLastSeenAt
                          ? <>{fmtDate(r.operatorLastSeenAt)} <span className="text-red-500 font-medium">({r.daysSinceLastLogin}d ago)</span></>
