...
type SortCol =
  | "name" | "city" | "plan" | "setupHealthScorePct" | "isPublished"
  | "daysSinceLastLogin" | "updatedAt" | "daysSinceActivation";
...
function PlanBadge({ plan }: { plan: string }) {
  ...
  return <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize ${cls}`}>{plan}</span>;
}
...
export default function ActiveStillOnboardingTable({ rows }: { rows: ActiveStillOnboardingRow[] }) {
  ...
  useEffect(() => {
    ...
    setSortDir(readUrlParam("dir", "asc") as "asc" | "desc");
  ...
  const sorted = useMemo(() => {
    return [...filtered].sort((a, b) => {
      ...
      switch (sortCol) {
        ...
        case "daysSinceLastLogin": cmp = (a.daysSinceLastLogin ?? -1) - (b.daysSinceLastLogin ?? -1); break;
        case "updatedAt":          cmp = a.updatedAt.localeCompare(b.updatedAt); break;
        case "daysSinceActivation": cmp = (a.daysSinceActivation ?? -1) - (b.daysSinceActivation ?? -1); break;
      }
  ...
  const handleExport = () => {
    const headers = ["Venue", "City", "Plan", "Health Score %", "Missing Setup Items", "Published", "Last Login", "Days Since Last Login", "Last Updated", "Claimed At", "Days Since Activation"];
    const csvRows = sorted.map((r) => [
      ...
      r.daysSinceLastLogin != null ? String(r.daysSinceLastLogin) : "",
      fmtDate(r.updatedAt),
      fmtDate(r.claimedAt),
      r.daysSinceActivation != null ? String(r.daysSinceActivation) : "",
    ]);
    downloadCsv(`active-still-onboarding-${new Date().toISOString().slice(0, 10)}.csv`, buildCsv(headers, csvRows));
  };
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-slate-50">
                    ...
                    <th className="text-left px-4 py-3"><button onClick={() => applySort("daysSinceLastLogin")} className={TH}>Last Login <SortIcon active={sortCol === "daysSinceLastLogin"} dir={sortDir} /></button></th>
                    ...
                    <th className="text-left px-4 py-3"><button onClick={() => applySort("daysSinceActivation")} className={TH}>Days Since Claim <SortIcon active={sortCol === "daysSinceActivation"} dir={sortDir} /></button></th>
                    ...
                    <tr key={r.id} onClick={() => router.push(`/control-panel/venues/${r.id}`)}
                      ...
                      <td className="px-4 py-3 text-gray-500 whitespace-nowrap text-xs">
                        {r.operatorLastSeenAt
                          ? <>{fmtDate(r.operatorLastSeenAt)} <span className="text-gray-400">({r.daysSinceLastLogin}d ago)</span></>
                      ...
                      <td className="px-4 py-3 text-gray-600 tabular-nums text-xs">
                        {r.daysSinceActivation != null ? `${r.daysSinceActivation}d` : <span className="text-gray-300">—</span>}
                      </td>
