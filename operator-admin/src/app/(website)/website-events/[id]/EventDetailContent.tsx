...
import type { WebsiteEventDetail } from "@/lib/data/events";
 ...
 * compatibility route ((website)/website-events/[id]/page.tsx, which only
 * renders this directly in the graceful-fallback case: an event whose venue
 * has no resolvable market/city yet). Kept as a single component so the two
 * routes can never drift into two different page layouts — see the event
 * URL architecture Phase 2 plan.
 *
 * `eventCanonicalPath` is the route-agnostic canonical path the CALLER has
 * already resolved (the real /{market}/{city}/events/{slug} path, or the
 * /website-events/{uuid} fallback when geography is unresolved) — used for
 * the breadcrumb JSON-LD and the Share button's URL. This component never
 * computes it itself, so it never has to know which route it's being
 * rendered from.
 */
            ...
            <div className="order-last lg:order-first flex flex-col gap-0 pt-0 lg:pt-2">
              ...
              <div className="flex items-start gap-2 mb-3">
                ...
                <div className="flex items-center gap-0.5 shrink-0 -mt-0.5">
                  ...
                  {/* eventCanonicalPath is the caller-resolved canonical path
                      — never the request's query/hash state — so this always
                      excludes preview/section params. */}
              ...
              <section id="more-events" style={{ scrollMarginTop: SCROLL_MARGIN }}>
                ...
                <div className="space-y-3">
                  {event.otherEvents.map((other) => {
                    ...
                    // Same venue as the main event, so it shares
                    // venueMarketSlug/venueCitySlug — falls back to the UUID
                    // compat route only if geography is unresolved (same
                    // condition that would apply to the main event itself).
                    const otherHref =
                      buildEventPublicPath({
                        marketSlug: event.venueMarketSlug,
                        citySlug: event.venueCitySlug,
                        eventSlug: other.slug,
                      }) ?? `/website-events/${other.id}`;
                    return (
                      <Link
                        key={other.id}
                        href={otherHref}
