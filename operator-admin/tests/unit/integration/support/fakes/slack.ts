...
export async function sendSlackAcquisitionNotification(p: { channel?: string; title?: string; message?: string }) {
  world.slack.push({ kind: "acquisition", channel: p.channel, title: p.title, message: p.message });
  return "delivered" as const;
}
