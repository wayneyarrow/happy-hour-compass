...
**Schema and pure-policy foundation only — nothing in this phase runs at runtime.** Migration `099_operator_activation_reminders.sql` is prepared but **not applied**. There is no cron, no orchestrator, no automated email, no Slack notification, and no expiry processing yet — the kill switch that will eventually gate live sending (`OPERATOR_ACTIVATION_REMINDERS_ENABLED`, mirroring `CUSTOMER_SUCCESS_EMAILS_ENABLED`) remains future work, not yet added.
...
- **No backfill.** Migration 099 changes zero existing rows — every live lifecycle, including the first real tracked one (Kelly Terris / Buffalo Rouge Brewing Co.), keeps `reminder_next_attempt_at = NULL` until a future, still-unbuilt orchestrator's first enabled pass lazily initializes it (`computeInitialNextAttempt()`).
