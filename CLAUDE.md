# Happy Hour Compass — Project Rules for Claude Code

## Repository layout

The active Next.js application lives entirely under **`operator-admin/`**. Every path below — `src/app/`, `src/lib/`, `app/(website)/`, `app/(consumer)/`, `app/admin/`, `app/control-panel/`, etc. — is relative to `operator-admin/` (e.g. "`src/lib/markets.ts`" means `operator-admin/src/lib/markets.ts`). The repository root also contains `docs/`, data CSVs, and a shared **`supabase/`** directory (migrations apply to the one Supabase project used by the whole app) — `supabase/` is the one exception and is *not* nested under `operator-admin/`.

## Product Architecture

### One engine, four presentation layers

HHC is a single product engine with multiple separate presentation layers. The engine is shared; the presentation layers must not be blended.

**Shared product engine** — lives in `src/lib/`, `src/app/api/` (both under `operator-admin/`), and the repo-root `supabase/`. Reuse freely across all presentation layers:
- Discover Engine: `src/lib/discover/discoverEngine.ts`, `featuredEventsEngine.ts`
- Data helpers: `src/lib/data/venues.ts`, `events.ts`, `discoverOverrides.ts`, etc.
- Market infrastructure: `src/lib/markets.ts`, `src/lib/activeMarket.ts`
- Analytics: `src/app/api/track/*`, `src/lib/analytics.ts`, `src/lib/trackingSession.ts`
- Business rules: `src/lib/plans.ts`, `src/lib/subscriptions.ts`, `src/lib/venueSetupStatus.ts`
- Supabase clients: `src/lib/supabase/server.ts`, `src/lib/supabase/browser.ts`
- Utilities: email, Slack, slugify, imageProcessing, seededTags, trustSignals
- Server Actions that write to shared tables (claim, operator submission, etc.)
- API routes: all `/api/*` routes are shared infrastructure

**The four presentation layers — each is separate. Do not blend them:**

#### 1. Public Website — `app/(website)/`
- SEO-first, responsive desktop layout, premium modern consumer experience
- Should feel like Airbnb, OpenTable, Resy, Spotify, or Apple
- Must NOT feel like admin software, a SaaS dashboard, or an app simulator
- Developed on the `website` branch → staging.happyhourcompass.com
- Eventually served at happyhourcompass.com
- Has its own layout, nav, footer — completely separate from the consumer app shell
- May reuse engine functions, data helpers, and individual UI components from `(consumer)` where they fit
- Must NOT use or modify `ConsumerLayout` (phone frame + `ConsumerNav`) as its shell

**Current implementation** (`app/(website)/`) includes:
- Homepage with hero section, plus assembled-Homepage rendering driven by the Collections/Homepages admin (`page.tsx`, `HeroSection.tsx`, `homepage/HomepageDiscoveryShell.tsx`, `HomepageSectionsRenderer.tsx`, `FeatureSection.tsx`, `CollectionRail.tsx`)
- Website header/footer, including location switching across markets/cities (`WebsiteHeader.tsx`, `WebsiteFooter.tsx`, `WebsiteLocationSwitcher.tsx`)
- Happy hour and event search, with results maps (`website-happy-hours/`, `website-events/`, `SearchResultsMap.tsx`), including market-aware homepage venue search
- Venue and event detail pages (`[market]/venue/[slug]/`, `website-events/[id]/`)
- Consumer accounts, saved venues/events, and save actions (`ConsumerAuthProvider.tsx`, `account/`, `saved/`, `SaveVenueButton.tsx`, `SaveEventButton.tsx`)
- Acquisition flows — suggest venue, add venue, contact us modals (`acquisition/`), plus a standalone guided Claim Your Venue onboarding flow (`claim-your-venue/`)
- Public guides with FAQ sections and schema markup (`[market]/guides/`, `[market]/guides/[slug]/`)
- Collection landing pages for venue, event, and editorial-guide collections (`collections/`)
- For Businesses landing page — hero, product tour, pricing, comparison table, funnel narrative (`business/`)
- About Us page (`about/`) and Careers page (`careers/`)
- Shared public 404 experience

#### 2. Consumer App — `app/(consumer)/`
- App-like experience: 375px phone frame on desktop, full-screen on mobile, bottom nav
- Currently served at happy-hour-compass.vercel.app (from `main` branch)
- May later inform native iOS/Android direction
- **Do NOT modify during website development unless explicitly intended.** Accidental changes here affect the production app on main.

#### 3. Operator Admin — `app/admin/`
- Business/venue management for operators
- Admin/SaaS UI style is appropriate here
- Auth-gated via middleware; operators only
- Keep completely separate from public website styling and layout

#### 4. Founder Control Panel — `app/control-panel/`
- Internal platform management for the founder
- Admin/SaaS UI style is appropriate here
- CP-admin allowlist gated
- Keep completely separate from public website styling and layout

---

### Website branch rules

1. **Website work happens on the `website` branch only.** Never commit website-specific changes to `main`.
2. **Website UI goes in `app/(website)/`.**
3. **The website has its own layout.** Do not add website routes or layout changes to `app/(consumer)/layout.tsx`, `app/(legal)/layout.tsx`, or `app/(standalone)/layout.tsx`.
4. **Reuse the engine.** Any function in `src/lib/` is fair game. Any API route is shared. Any data helper is shared.
5. **Components: import, don't modify.** If a `(consumer)` component fits the website, import it as-is. Adapt it in a wrapper or a new component in `(website)/`. Do not edit the `(consumer)` component to accommodate website needs.

---

### Staging and deployment

- `main` branch → Vercel Production → happy-hour-compass.vercel.app (the consumer app, operator admin, control panel)
- `website` branch → Vercel Preview → staging.happyhourcompass.com (the public website)
- **No Vercel build config changes are needed.** Staging vs production behavior is controlled entirely through Next.js routing on the respective branches and the `NEXT_PUBLIC_SITE_URL` / `NEXT_PUBLIC_NOINDEX` env vars already in place.
- When the website is ready for launch, the routing and domain assignment changes in Vercel dashboard — not in the codebase.

---

### Current status and next steps

- Content Engine (guide publishing) Phase 1 and the public Guide Experience V2 (editorial layout, FAQ sections with schema markup) are complete.
- Collections Management V1 (control-panel CRUD for Collections, resolved/preview tables, guide picker) is complete, including the algorithmic collection workflow and resolved-collection UX.
- Homepage Management is complete: the control-panel admin UI for assembling Collections into Homepages (creation flow, sections editor, content/guide pickers, preview) is built (`app/control-panel/homepages/`), and the public site renders assembled Homepages and Collection landing pages. See `docs/website/HOMEPAGE_COLLECTIONS_PRODUCT_SPEC.md`.
- Several new public-facing pages have shipped since Homepage Management: For Businesses landing page, About Us, Careers, and a standalone guided Claim Your Venue onboarding flow.
- SEO/metadata implementation (canonical URLs, Open Graph/Twitter, structured data, sitemap, robots, environment-aware indexing) is complete for launch, with one known pre-launch gap around market-status gating in the sitemap. See the **SEO & Metadata** section below and `docs/website/SEO_ROADMAP.md`.
- Next major implementation area is not yet defined in this doc — confirm with the user/product docs before starting new large website work.

---

### Reference documents

- `operator-admin/WEBSITE_REUSE_AUDIT.md` — detailed audit of what to reuse, what to adapt, what to build new; recommended build sequence; key risks
- `docs/PHASE2-VENUE-COMPLETENESS.md` — venue completeness signals

---

## Website Vision & Playbook

Before any significant public-facing website functionality — UI, UX, layout, navigation, homepage, search, filters, maps, mobile experience, SEO, geographic or location architecture, content, or architecture decisions — review all five documents:

1. **[`docs/website/WEBSITE_VISION_AND_DESIGN_PRINCIPLES.md`](docs/website/WEBSITE_VISION_AND_DESIGN_PRINCIPLES.md)** — Website philosophy, UX principles, and design vision. Takes precedence over implementation convenience.
2. **[`docs/website/WEBSITE_PRODUCT_PLAYBOOK.md`](docs/website/WEBSITE_PRODUCT_PLAYBOOK.md)** — Engineering implementation guide and website build standards. Defines how to apply the philosophy during real implementation. Includes the **Geographic Information Architecture** — the authoritative reference for market, city, neighbourhood, URL structure, search origin priority, and all geographic decisions. Review this section before any geographic, URL, SEO, or location-related implementation.
3. **[`docs/website/CONSUMER_EXPERIENCE_PRD.md`](docs/website/CONSUMER_EXPERIENCE_PRD.md)** — Consumer product experience, customer journey, homepage philosophy, search framework, search results framework, filters, maps, mobile experience, and future consumer roadmap.
4. **[`docs/website/CONTENT_ENGINE_PRODUCT_SPEC.md`](docs/website/CONTENT_ENGINE_PRODUCT_SPEC.md)** — The Happy Hour Compass Content Engine is the long-term publishing engine for the public website's editorial content (Venue Guides, Event Guides, and future guide types). It is not a traditional CMS or blogging platform. It is a core architectural document, equal in importance to the Website Vision and Website Product Playbook. Review it before any Content Engine (CMS) implementation work — new guide types, publishing workflow changes, SEO automation, distribution, or admin control panel content functionality.
5. **[`docs/website/HOMEPAGE_COLLECTIONS_PRODUCT_SPEC.md`](docs/website/HOMEPAGE_COLLECTIONS_PRODUCT_SPEC.md)** — The canonical reference for Homepage Management and Collections: the Content → Collections → Homepages ownership model, geography-aware curation, Homepage Sections/Templates/Fallback, and how existing Discover Management logic evolves into Collections. Review it before any Homepage, Collection, or editorial merchandising implementation work.

If a requested implementation conflicts with any of these documents, surface the conflict before writing code.

Key principles from these documents to carry into every website task:
- The website is the product, not a marketing website. Discovery always comes before explanation.
- Reveal and elevate the existing product engine — never rebuild what already exists.
- Every homepage section must earn its place by improving discovery; it should feel like the beginning of the product, not an introduction to it.
- Show the product instead of describing it. Real venues, real events, real content.
- The product should feel closer to Airbnb, Apple, OpenTable, Resy, or Spotify — never like SaaS or admin software.
- Desktop must feel immersive; mobile must feel native. Neither is an afterthought.
- Do not create UI backed by immature or inconsistently populated data.
- Preserve separation between Public Website, Consumer App, Operator Admin, and Founder Control Panel.
- The Content Engine is the long-term publishing engine for the public website — not a traditional CMS or blogging platform. Follow `docs/website/CONTENT_ENGINE_PRODUCT_SPEC.md` before introducing new Content Engine functionality or architecture; surface conflicts before writing code.
- Homepages assemble Collections; Collections own editorial curation; content never knows where it's displayed. Follow `docs/website/HOMEPAGE_COLLECTIONS_PRODUCT_SPEC.md` before introducing new Homepage, Collection, or editorial merchandising functionality or architecture; surface conflicts before writing code.
- All new or edited Happy Hour Compass consumer guides (Content Engine venue/event guides) must follow `docs/website/GUIDE_SEO_STANDARD.md`. Consult it whenever creating, editing, reviewing, or auditing a guide — SEO is part of guide creation from the start, not a later add-on. Guide-specific SEO/content values belong in the Content Engine guide record and must stay visible/editable in the Founder Control Panel — never hard-code guide metadata in application code. Guide work follows the same workflow as everything else: discuss → task → implementation → report → review/QA → explicit commit only.

---

## SEO & Metadata

The public website's SEO/metadata implementation is complete for launch (see `docs/website/SEO_ROADMAP.md` for full status, deferrals, and the launch-day checklist). The rules below are load-bearing — follow them for any new public page rather than re-deriving the pattern.

### Architecture

- `operator-admin/src/lib/siteUrl.ts` — single source of truth for absolute URLs and noindex logic (`getSiteUrl()`, `shouldNoIndex()`, `absoluteUrl()`). Every other SEO file reads through these three functions; nothing else reads `NEXT_PUBLIC_SITE_URL` / `NEXT_PUBLIC_NOINDEX` directly.
- `operator-admin/src/lib/seo/metadata.ts` — `buildPageMetadata()`, the shared metadata helper, plus `buildVenueMetadata()` / `buildEventMetadata()` (thin wrappers for those two route shapes).
- `operator-admin/src/lib/seo/schema/` — JSON-LD structured-data builders, one per entity type, rendered through the shared `<JsonLd>` component (`(website)/JsonLd.tsx`).
- `operator-admin/src/app/sitemap.ts` / `robots.ts` — sitemap and robots generation, reusing the same visibility rules (`is_published`, `status='published'`, etc.) the pages themselves already use to decide whether to render or 404.

### buildPageMetadata() — use it for every new public page

Call `buildPageMetadata({ title, description, path, ogImage?, ogTitle?, ogDescription?, ogType? })` rather than hand-writing a `metadata` object. It generates canonical, Open Graph, and Twitter Card metadata together and applies environment-aware `noindex` automatically. Static pages assign it directly (`export const metadata = buildPageMetadata({...})`); dynamic pages call it from `generateMetadata()`. Every current public page (Homepage, venue, event, guide, guide index, collection, About, Business, Careers, Claim Your Venue, Privacy, Terms) goes through this helper — do not hand-roll a competing canonical/OG/Twitter implementation for a new one.

### Title template convention

The root layout (`operator-admin/src/app/layout.tsx`) sets `title: { template: "%s — Happy Hour Compass", default: "Happy Hour Compass" }`.
- **Default:** return a plain-string title with no brand suffix — the template appends "— Happy Hour Compass" exactly once. This is what `buildPageMetadata()` expects.
- **Exception:** use `title: { absolute: "..." }` only when the title is already fully composed elsewhere (CMS/admin-authored — currently only Homepage and Guide detail, where the stored `page_title` may already include the brand). This bypasses the template. Don't reach for it as a shortcut on a new page.
- Never hardcode "Happy Hour Compass" into a plain-string title — it will render twice.

### Canonical URL conventions

- Every indexable public page sets a self-referencing canonical via `buildPageMetadata()`'s `path` (or `canonicalPath` for a CMS-driven override, e.g. a guide's admin-entered `canonical_url`).
- A canonical must always point at a URL that actually resolves to a real route — never assert one for a path with no corresponding `page.tsx`.
- **Homepage canonical is always `/`, regardless of resolved market/city.** `WEBSITE_PRODUCT_PLAYBOOK.md`'s Geographic Information Architecture documents a future `/{market}/{city}` canonical shape, and the Homepage CMS's `canonicalUrl` field (`homepageSeo.ts`, `control-panel/homepages/HomepageForm.tsx`) is already built against it — but no `/{market}` or `/{market}/{city}` route exists yet. Until those routes ship, `(website)/page.tsx`'s `generateMetadata()` intentionally ignores the CMS-stored value and always resolves to `/`. Do not wire the CMS canonical back into the live tag without first shipping the routes it depends on.

### Open Graph / Twitter conventions

- `buildPageMetadata()` sets `openGraph` and `twitter` together, always, for every caller. Don't add a page-specific OG/Twitter block outside the helper.
- **Default Open Graph image:** `DEFAULT_OG_IMAGE` in `metadata.ts` is `/logo.png` — the sitewide brand asset already used elsewhere (Organization JSON-LD, email templates, auth pages). It's used automatically whenever a page doesn't pass its own `ogImage`. Never reference `/og-default.png` — that path was never a real file and has been fully removed from the codebase.
- Pages with a real content image (venue photo, event photo, guide hero image) should pass it as `ogImage`; everything else correctly falls through to the shared default.

### Structured data (JSON-LD)

- Builders live in `src/lib/seo/schema/` — one per entity (`organization.ts`, `website.ts`, `breadcrumb.ts`, `venue.ts`, `article.ts` for guides, `collection.ts`, `aboutPage.ts`, `faq.ts`). Each returns a plain `SchemaNode | null`; never include `@context`/`@graph` inside a builder — that envelope belongs solely to `<JsonLd>`.
- Organization + WebSite render once, sitewide, from `(website)/layout.tsx`. Every other node is page-specific.
- Governing rule for every builder: never mark up a fact the data model can't stand behind — omit a property entirely (never a guessed or placeholder value) when the underlying data isn't reliably available at scale.
- **Event structured data (`schema.org/Event`) is intentionally deferred.** The current event data model cannot produce a truthful, timezone-aware occurrence datetime, which `Event` schema requires. Do not add it until that gap is actually resolved — see `docs/website/SEO_ROADMAP.md`.

### Environment-aware indexing

- `shouldNoIndex()` drives both `robots.txt` (full `Disallow: /` on staging) and each page's `<meta name="robots">`. Controlled by `NEXT_PUBLIC_NOINDEX=true`, set in Vercel project settings for the staging/preview environment only — **never set it on Production.**
- `NEXT_PUBLIC_*` env vars are inlined at build time, not read at request time. Changing this value requires a new build/deploy — a runtime-only restart will not pick up the change.

### Implementation rules for future contributors

- Use `buildPageMetadata()` for every new public page; don't hand-write canonical/OG/Twitter.
- Never hardcode the brand suffix into a plain-string title.
- Never reference `/og-default.png`.
- Never add a canonical pointing at a route that doesn't exist yet.
- Before adding Event structured data, confirm the timezone/occurrence-datetime gap is actually closed.
- See `docs/website/SEO_ROADMAP.md` for completed work, intentional deferrals, known gaps, and the launch-day checklist.

---

## Authentication & Email

### Consumer email confirmation flow

Consumer signup confirmations resolve through **`/auth/confirm`** (`operator-admin/src/app/auth/confirm/page.tsx`), not `/auth/callback`.

**Rule:** Never route a consumer confirmation link through `/auth/callback`.

**Why:** `/auth/callback` handles the browser PKCE `?code=` flow. Server-generated Supabase links (`auth.admin.generateLink()`, used for consumer signup) have no browser `code_verifier` to anchor a PKCE exchange, so Supabase can only redirect back with tokens in the URL hash fragment (`#access_token=...&type=signup`) — a shape `/auth/callback` doesn't understand. `/auth/confirm` is the dedicated handler for that hash-fragment shape.

### Supabase Redirect URLs

**Rule:** Every authentication callback route the app redirects to (`/auth/callback`, `/auth/confirm`, `/operator/create-password`, etc.) must also be added to the Supabase dashboard under **Authentication → URL Configuration → Redirect URLs** — for **both** the staging and production URLs. Adding the route in code is not sufficient by itself.

**Why it matters:** If a `redirectTo` isn't in that allow-list, Supabase does not error — it silently falls back to the dashboard-configured Site URL, stripping the intended path/query entirely. This produces misleading behavior: the code is correct, but the deployed redirect is wrong, and it looks like the new route "isn't being used." See `operator-admin/src/app/(consumer)/RecoveryRedirect.tsx`'s header comment for the original diagnosis of this failure mode (operator recovery links).

### Environment-aware authentication links

**Rule:** Every authentication email must build its redirect/action URL through `getSiteUrl()` (`operator-admin/src/lib/siteUrl.ts`) rather than a hard-coded domain. This is the same environment-aware helper used for SEO/metadata — it resolves to the correct staging vs. production origin per deployment, whereas Supabase's own dashboard-configured Site URL does not vary per environment.

### Supabase Auth email delivery (SMTP)

No flow in the app relies on Supabase Auth's own mail delivery anymore. Every Auth-adjacent email — consumer signup confirmation, operator activation, operator forgot-password, and consumer forgot-password (`requestConsumerPasswordReset`, `operator-admin/src/app/(consumer-auth)/account/forgot-password/actions.ts`) — calls `supabase.auth.admin.generateLink()` to obtain the link only, then sends its own branded email via Resend (`src/lib/email.ts`), deliberately bypassing Supabase's mailer.

Consumer forgot-password used to be the one exception: it called `supabase.auth.resetPasswordForEmail()` and let Supabase render and send the whole email itself. Staging end-to-end testing (2026-07-24) traced Gmail's "This message might be dangerous" warning and the missing Slack/branding on that email directly to this — the email came from Supabase's own unauthenticated mail path with Supabase's default unbranded template, not from Resend/`hello@happyhourcompass.com`. It was switched to the `generateLink()` + Resend pattern below to match every other flow; see that file's header comment for the full reasoning, including how it still avoids embedding the raw `action_link` (see the token_hash note further down).

**Rule:** Supabase Auth should still have **Custom SMTP enabled** (Supabase dashboard → Authentication → Emails → SMTP Settings), routed through the project's existing Resend account, using the domain already verified there (`happyhourcompass.com` — SPF/DKIM confirmed via Resend + DNS), as defense-in-depth in case any future flow ever falls back to Supabase's own mailer (e.g. a new flow calling `resetPasswordForEmail()` or `signUp()` directly). No current flow depends on it, but leaving Supabase's shared/default mail infrastructure as the fallback is never correct for production (low fixed send-rate limits).

**Why it matters:** This is a single project-wide Supabase setting — there is only one Supabase project behind local dev, staging, and production (see `supabase/` note in Repository layout) — so it can't be scoped per environment, and misconfiguring or reverting it affects every environment identically, including local dev.

**Required values** (Resend's standard SMTP relay — see Resend's SMTP docs for the authoritative values, don't hardcode them elsewhere): host `smtp.resend.com`, username `resend`, password = the existing `RESEND_API_KEY`, sender `hello@happyhourcompass.com` / "Happy Hour Compass" — matching `DEFAULT_FROM` in `src/lib/email.ts`. No new DNS records are needed; the domain is already verified in Resend.

**Custom SMTP alone does not resolve Gmail's "This message might be dangerous" warning.** That warning was confirmed (via Gmail "Show original" evidence) to be unrelated to sender authentication — SPF, DKIM, and DMARC all pass and are fully aligned to `happyhourcompass.com` once Custom SMTP is enabled. The actual trigger is the **link inside the email**: Supabase's default recovery template builds the link from `{{ .ConfirmationURL }}`, which renders as a raw `https://<project-ref>.supabase.co/auth/v1/verify?...` URL — a branded sender pointing at an opaque, unrelated third-party domain, which is exactly the pattern Gmail's phishing heuristics flag regardless of passing authentication. The fix is a template + app change, not an SMTP/DNS change:
- The Supabase **Reset Password email template**'s `{{ .RedirectTo }}?token_hash={{ .TokenHash }}&type=recovery` shape (instead of `{{ .ConfirmationURL }}`) is what this pattern is named after — it keeps the link on our own domain instead of the raw Supabase project domain. This dashboard template is no longer actually invoked by the app (nothing calls `resetPasswordForEmail()` anymore, so Supabase never renders it), but the same link shape is now built directly in code: `requestConsumerPasswordReset` calls `admin.generateLink({ type: "recovery" })` and rebuilds the link as `${redirectTo}?token_hash=${linkData.properties.hashed_token}&type=recovery` — `hashed_token` is the same value `{{ .TokenHash }}` would have resolved to. Do **not** switch this to `linkData.properties.action_link` — that's the raw `/auth/v1/verify` URL and reintroduces the prefetch-consumption risk below.
- `/account/reset-password` (`operator-admin/src/app/(consumer-auth)/account/reset-password/page.tsx`) handles the resulting `?token_hash=...&type=recovery` query param, alongside the pre-existing hash-fragment (`access_token`/`refresh_token`) handling kept for backward compatibility.
- **Consumer signup confirmation, operator activation, and operator forgot-password carry the same underlying risk** — all three embed a Supabase `admin.generateLink()` `action_link` directly into their branded Resend emails, and `action_link` is the same raw `/auth/v1/verify` URL shape. None have triggered a reported Gmail warning, so none have been changed — but if one ever does, the fix is the same class (build the link from `{{ .RedirectTo }}`/`{{ .TokenHash }}`-equivalent data rather than the raw action link), not an SMTP/DNS change. Consumer forgot-password is deliberately no longer in this list — see the token_hash bullet above.

**A one-time recovery token must never be verified automatically on page load.** Supabase documents email-security scanners prefetching links as a common cause of single-use recovery tokens being silently consumed before the user ever opens the message — confirmed here directly: a fresh, never-manually-reused `token_hash` sometimes already read as expired (`verifyOtp` → `code: otp_expired`, HTTP 403) purely from the page having loaded. This is not fixed by the user "clicking faster" — the token can be consumed by something other than the user's own click at any point after the email is sent, regardless of how quickly they open it — and a copy-the-link-and-paste-it-elsewhere affordance doesn't fix it either, since loading the URL at all is the trigger, not which application renders it.

**Rule:** `/account/reset-password` must never call `supabase.auth.verifyOtp()` from `useEffect`, initial render, or any other page-load/navigation/prefetch path. A `token_hash` link lands on an intermediate branded confirmation state ("Reset your password" / "Continue to securely reset the password for your Happy Hour Compass account." / a "Continue to reset password" button) and `verifyOtp()` is called **only** from that button's explicit `onSubmit`/click handler — guarded both by disabling the button while the request is in flight and by an in-memory ref checked synchronously (`verifyingRef`), so neither a double-click nor a React re-render can issue a second verification call for the same token. Reloading the confirmation page must never consume the token either — it only re-detects the `token_hash` query param, it never re-verifies.

### Known Supabase generateLink() JWT/kid failures

Supabase's GoTrue Admin API intermittently rejects `auth.admin.generateLink()` calls with `"unrecognized JWT kid <nil> for algorithm ES256"` — a confirmed upstream bug following Supabase's platform-wide HS256→ES256 JWT signing-key migration (see supabase/supabase#48295, #48272), not a config or code issue on our side. It surfaced first in operator provisioning (`provisionOperatorForVenue`, `operator-admin/src/lib/operatorActivation.ts`) and again in the operator forgot-password flow (`forgotPasswordAction`) during 2026-07-24 staging testing — a request that hits it gets no recovery link and no email, with only a Slack alert (if the call site alerts on `linkError`) as a symptom.

**Mitigation:** retry only this specific error, up to 3 attempts with short backoff; any other `generateLink` error still returns immediately on the first attempt. The shared helper is `generateLinkWithRetry()` (`operator-admin/src/lib/supabase/generateLinkWithRetry.ts`), used by `forgotPasswordAction` (operator) and `requestConsumerPasswordReset` (consumer). `provisionOperatorForVenue` keeps its own inline copy of the same check predating this helper — both are expected to be deleted once Supabase ships a fix, so they weren't consolidated. Any **new** `generateLink()` call site should use the shared helper rather than calling `supabase.auth.admin.generateLink()` directly.

### Operator activation lifecycle (Phase 1A foundation — 2026-09)

Claims (`venue_claims`) and Add Your Venue submissions (`operator_submissions`) share one operator-activation lifecycle: both provision an operator via the same `provisionOperatorForVenue()` and wait for `operators.account_activated_at` to be set. Migration `098_operator_activation_lifecycle_foundation.sql` (**applied**) adds the schema this lifecycle needs — **foundation only; the rules below describe what exists today, not the full planned system**:

- `public.operator_activation_lifecycles` is the **canonical** lifecycle table — the single source of truth for `started_at`/`deadline_at`/`reminder_stage`/`expired_at`/`released_at`. `venue_claims` and `operator_submissions` store **no** duplicated activation-lifecycle columns of their own; a lifecycle links back to its origin via `origin_type` (`'claim'` or `'submission'`) plus exactly one of `origin_claim_id`/`origin_submission_id` (enforced by a CHECK — never both, never neither).
- **At most one *live* lifecycle per operator, enforced by the database**, not application logic: a partial unique index on `operator_id` (`WHERE expired_at IS NULL AND released_at IS NULL`). Claiming a lifecycle (`claimOrReuseActivationLifecycle()`, `src/lib/activation/activationLifecycle.ts`) is a single atomic INSERT attempt — success means this call legitimately started a new lifecycle; a `23505` (unique-violation) conflict means another call already owns the live lifecycle for this operator, so the code reads back and reuses that winning row rather than creating a competing one. There is no separate "check if one exists" read before the write — the insert attempt *is* the check, which is what makes this safe under concurrent requests (see that file's header for the exact race this replaced).
- Already-activated operators (`operators.account_activated_at` set) are excluded before any insert is attempted — no lifecycle is ever created for them, regardless of origin.
- An expired or released lifecycle does **not** block a fresh one — the live-only partial index means a genuinely new claim/submission (a different origin) can start its own lifecycle once the operator's prior one is closed out. It does not permit restarting the *same* origin's lifecycle a second time (each origin is limited to at most one lifecycle row, ever).
- `event_type` / `metadata_json` on `venue_claim_notes` and `operator_submission_notes` — structured Internal Note support. Only `"activation_started"` and `"account_activated"` are actually written today (from the four `provisionOperatorForVenue()` call sites' post-claim step and `completeOperatorAccountActivation()` respectively); the rest of the planned vocabulary (`reminder_sent`, `activation_expired`, etc. — see `src/lib/activation/activationEvents.ts`) exists only as reserved TypeScript union members, not live events.
- Structured system notes are attributed via `created_by_email = "Happy Hour Compass"` (`SYSTEM_AUTHOR_EMAIL`, `src/lib/activation/activationNotes.ts`), not via any Control Panel rendering change — see that file's header for why.
- `src/lib/activation/activationState.ts`'s `deriveActivationState()` is the single place lifecycle state is derived — always from `operators.account_activated_at` plus a lifecycle row's own columns, never from whether an email/code was sent. No second stored "status" column exists or should be added. See the Phase 1B entry below for the full current state enum (Phase 1A shipped a smaller one).
- **Known narrow edge case:** `claimOrReuseActivationLifecycle()`'s own "is this operator already activated" check and its lifecycle INSERT are two separate statements, not one transaction — see that function's inline comment for the resulting (very narrow, non-duplicating) race and the reminder-query invariant it requires.

**Not yet implemented as of Phase 1A (do not assume otherwise):** activation reminder emails, a reminder cron, expiry/release execution, Control Panel visibility for this lifecycle, the six-digit OTP verification flow, and auto-approval changes to claim/submission routing. Control Panel visibility and two founder controls shipped in Phase 1B — see below; everything else in this list is still unimplemented after Phase 1B too.

### Operator activation lifecycle — Control Panel visibility & founder controls (Phase 1B foundation — 2026-09)

Phase 1B adds founder-facing visibility and two narrow founder actions on top of the Phase 1A schema above — it does **not** add reminders, OTP, auto-approval, or any automated expiry/release execution.

- **Presentation states** — `deriveActivationState()` (`src/lib/activation/activationState.ts`) now derives eight states: `not_tracked`, `awaiting_setup`, `expiring_soon`, `release_required`, `active`, `expired`, `released`, `setup_delivery_error`. `setup_delivery_error` is reserved for a future phase with an authoritative, persisted delivery-failure signal — no such column exists yet, so `deriveActivationState()` never actually returns it today.
- **Release Required vs. Expired — the one deterministic rule:** "Release Required" means the deadline has passed in wall-clock time but nothing has formally marked the lifecycle `expired_at` yet — which, since nothing in the codebase sets `expired_at` automatically, is what *every* currently-overdue lifecycle looks like. "Expired" means a (future) automated process already set `expired_at`. The two never overlap: `expiredAt` is checked before the deadline-passed check, so a lifecycle moves from Release Required to Expired (never both, never neither) the moment a future expiry job starts running.
- **N+1-safe data access** — `src/lib/activation/activationPresentation.ts` (server-only, admin-client) batches Claim/Submission list pages into a small, fixed number of queries regardless of row count — never one query per row. (As of Phase 1C this is no longer always exactly 2: an origin with no lifecycle row also gets its operator resolved — via venue linkage for Claims, the direct `operator_id` FK for Submissions — so an already-activated legacy operator can be distinguished from a genuinely untracked one; see the Phase 1C entry below.) Single-item wrappers (`getActivationPresentationForClaim`/`ForSubmission`) reuse the same batch path for detail pages. Never creates a lifecycle merely by being read. Not imported by any public/consumer/website route.
- **Claims and Submissions list pages** show an Activation badge + deadline countdown per row (a *separate* dimension from each flow's own approval/routing-status badge — never blended), a combined status/activation filter dropdown, and an attention-count strip (Release Required / Expiring Soon / Awaiting Setup) computed from the same authoritative batch data. Default sort is attention-first (Release Required → Expiring Soon → Awaiting Setup, each by nearest deadline → everything else, newest first) — clicking any other column header overrides it exactly as before.
- **Claim and Submission detail pages** gained a shared `ActivationCard` (`src/components/ActivationCard.tsx`) showing lifecycle state, operator, origin, start/deadline/remaining time, a static "No automated reminders sent" line, activated/expired/released timestamps when set, and the last structured event — rendered only for approved claims/submissions, and only ever reads existing data (never backfills or creates a lifecycle).
- **Internal Notes** render a friendly label for `event_type` (`getActivationEventLabel()`, `src/lib/activation/activationEvents.ts`) plus a small allowlisted subset of `metadata_json` (`src/components/ActivationNoteMeta.tsx`) — never the raw JSON blob. Neither Claims nor Submissions notes have ever had an edit/delete affordance in the UI, so "system notes are non-editable" was already true by omission; Phase 1B did not add one.
- **Resend setup email** (`resendClaimSetupEmailAction`/`resendSubmissionSetupEmailAction`) now additionally: confirms the operator is still unactivated, blocks a released lifecycle outright, and blocks (rather than silently sending against) a lifecycle whose deadline has already passed — the founder must extend the deadline first, since resend deliberately never auto-extends. A short retry-window check (reusing the `manual_resend` note it already writes) suppresses an accidental double-submit. On success it now writes one structured `manual_resend` note (recipient, timestamp, lifecycle id, current deadline — never the generated link/token) attributed to the real signed-in founder, not the "Happy Hour Compass" system author, since this is a human-triggered action.
- **Extend deadline by 7 days** (`extendActivationDeadlineAction`, `src/lib/activation/activationLifecycleActions.ts`) is a new, origin-agnostic, founder-only (`isControlPanelAdmin`) action shared by both detail pages. Rules: operator must be unactivated, lifecycle must not be released (never clears `released_at`); if the deadline is still future, +7 days from the current deadline; if it has passed, +7 days from the extension's own timestamp (`computeExtendedDeadline()`, `src/lib/activation/activationState.ts`). The update is an atomic compare-and-swap (`.eq("deadline_at", currentDeadlineAt).is("released_at", null)`) so two concurrent extensions can't lose an update — the loser gets a clear "refresh and retry" error instead of silently clobbering or double-extending. Writes one structured `deadline_extended` note, attributed to the real founder for the same reason as resend above.
- `writeActivationNote()` (`src/lib/activation/activationNotes.ts`, system-author attribution) is reserved for genuinely automated events (`activation_started`, `account_activated`). Founder-triggered structured events (`manual_resend`, `deadline_extended`) are written directly with the real founder's `created_by`/`created_by_email` instead — do not route a human-triggered event through `writeActivationNote()`.

**Still not implemented after Phase 1B (do not assume otherwise):** activation reminder emails, a reminder cron, automated expiry/release execution, manual venue release/reversion, the six-digit OTP verification flow, and any auto-approval or routing changes to claims/submissions.

### Operator activation lifecycle — controlled legacy activation resume (Phase 1C foundation — 2026-09)

Migration 098 deliberately performed no backfill, so every historical Claim/Submission approved before Phase 1A shipped has no `operator_activation_lifecycles` row. Phase 1C adds a narrow, founder-controlled way to opt **one** such legacy operator into tracking at a time — **explicitly not a bulk backfill**, and no migration was required for it.

- **One record at a time, founder-triggered.** From an eligible Claim/Submission detail page's Activation card, a founder can click "Start activation tracking & resend setup email" — `resumeLegacyClaimActivationAction`/`resumeLegacySubmissionActivationAction` (`src/lib/activation/legacyActivationResumeActions.ts`, thin fixed-signature wrappers) → `legacyActivationResumeImpl.ts` (the real logic, no `"use server"`, DI seam for tests only). Eligibility requires: founder authorization, an unambiguous operator resolved via the audited venue-linkage rules, `account_activated_at IS NULL`, no lifecycle row for this exact origin in any state (migration 098's origin-uniqueness index makes a second one permanently impossible), and no live lifecycle for the operator elsewhere.
- **Fresh 14-day lifecycle + immediate setup email**, via the same atomic `claimOrReuseActivationLifecycle()` and setup-email templates every other flow uses. **Existing activated legacy operators are excluded** — an operator with `account_activated_at` already set and no lifecycle row now correctly presents as `active` ("Active — account activated before lifecycle tracking" on the card, no Start-tracking button), never `not_tracked`. This was a genuine, pre-existing gap in Phase 1B's presentation layer, not new Phase 1C behavior — `getActivationPresentationsForClaims`/`ForSubmissions` previously never resolved an operator at all for an origin with no lifecycle row.
- **Only a newly started lifecycle sends the email.** `claimOrReuseActivationLifecycle()`'s `"reused"` decision — whether it points at the same origin (a retried/duplicate request) or a different one (a live lifecycle already exists elsewhere for the operator) — never sends an email and never writes a note. This is what makes concurrent requests and a plain retry of this action safe; it does **not** prove Resend actually delivered the one email a successful `"started"` call did attempt when the provider's own response is ambiguous or times out — that narrow external-provider gap could still let a later manual Resend duplicate an already-delivered email. See `legacyActivationResumeImpl.ts`'s header comment for the precise scope of this guarantee.
- Writes exactly one structured `legacy_activation_resumed` note, founder-attributed (never the "Happy Hour Compass" system author, matching `manual_resend`/`deadline_extended`), with only `lifecycleId`/`recipient`/`deadline`/`flow` — never a link or token.

### Operator activation lifecycle — reminders & expiry review (Phase 2A-2 foundation — 2026-09)

**Schema and pure-policy foundation.** Migration `099_operator_activation_reminders.sql` is applied. See the Phase 2A-3 entry below for the worker that now reads/writes this schema (live in Production).

- **Reminder timing is deadline-relative, never start-relative**: Reminder 1 = `deadline_at - 11 days`, Reminder 2 = `deadline_at - 7 days`, Reminder 3 = `deadline_at - 2 days`, expiry review = `deadline_at`. Anchoring to the deadline (never to `started_at`, which an extension never moves) is what lets `extendActivationDeadlineImpl.ts` recompute reminder timing correctly with no special-casing. See `src/lib/activation/activationReminderPolicy.ts` (pure, no I/O) for the full timing/catch-up/extension-resolution logic.
- **`reminder_stage` means "highest stage durably resolved," not "reminders actually sent."** A stage resolves three ways: its reminder was actually delivered; a later stage's reminder was sent instead, silently superseding it (the catch-up policy never sends more than one reminder per processing pass — see `selectCatchUpStage()`); or a deadline extension/reopen moved the deadline far enough that the stage's original timing became moot before it was ever processed (see `computeExtensionResolution()`). **Only the corresponding `reminder_sent` Internal Note proves an actual delivery** — never assume a stage was delivered from `reminder_stage` alone.
- **Deadline extension preserves completed stages.** `extendActivationDeadlineImpl.ts` no longer blindly resets `reminder_stage` to 0 on every extension/reopen (the pre-Phase-2A behavior) — it now calls `computeExtensionResolution()` to preserve every already-resolved stage, silently skip (never send for) any stage whose new due time is already in the past relative to the extension's own transaction time, and schedule `reminder_next_attempt_at` for the earliest stage still strictly in the future (or `null` if stage 3 is already resolved — an extension never schedules further automatic reminders past that point; the founder may use manual Resend independently).
- **Extension refuses to run while a reminder lease is active.** The extension's compare-and-swap now also pins `reminder_lease_started_at IS NULL` — if a (future) reminder worker currently owns this lifecycle's lease, the extension makes no state change at all and returns "A reminder is currently being processed. Please refresh and try again shortly." It never clears or steals an active lease; only a future worker's own stale-lease recovery may do that.
- **New columns on `operator_activation_lifecycles`** (migration 099): `reminder_next_attempt_at`, `reminder_attempt_count`, `reminder_last_attempted_at`, `reminder_last_error`, `reminder_lease_stage`/`reminder_lease_started_at` (a lease pair — both null or both non-null, enforced by a CHECK), `expiry_slack_notified_at`, `expiry_founder_email_sent_at`. `reminder_stage` itself is now CHECK-bound to exactly 0-3 (widened from the original unbounded `>= 0`).
- **New `event_key` column on `venue_claim_notes` and `operator_submission_notes`**, each with a partial unique index (`WHERE event_key IS NOT NULL`) — a stable, deterministic identifier (e.g. `hhc-activation-reminder:<lifecycleId>:<stage>`, `hhc-activation-expiry:<lifecycleId>`) that will let a future reminder/expiry worker's note-insert be safely retried (a duplicate `event_key` insert fails with `23505`, treated as "already written, not an error"). NULL for every existing note.
- **No backfill.** Migration 099 changed zero existing rows — every live lifecycle, including the first real tracked one (Kelly Terris / Buffalo Rouge Brewing Co.), kept `reminder_next_attempt_at = NULL` until the Phase 2A-3 worker's lazy-init step (below) first ran against it.

### Operator activation lifecycle — reminder/expiry worker (Phase 2A-3 — 2026-09)

**The worker is fully wired (cron route, `vercel.json` schedule, dry-run script) and LIVE in Production**: `isOperatorActivationReminderProcessingEnabled()` (`src/lib/activation/activationReminderConfig.ts`) checks `process.env.OPERATOR_ACTIVATION_REMINDERS_ENABLED === "true"` as the literal first line of `processActivationReminders()`, and `OPERATOR_ACTIVATION_REMINDERS_ENABLED=true` is active in Production. Production evidence confirms real delivery: `reminder_sent` Internal Notes exist for Buffalo Rouge Brewing Co. (stage 1, 2026-09-22) and Table 19 (stage 1, 2026-09-25). Unset or any other value still returns immediately with zero reads, writes, or external calls. Changing the variable, the worker, or reminder behavior requires separate explicit authorization — and the email-code activation initiative (below) must not change existing lifecycle/reminder behavior.

- **Orchestrator** (`src/lib/activation/processActivationReminders.ts`) runs, in order: stale-lease recovery (leases held >15 minutes, mirroring Customer Success's own threshold) → lazy initialization of any live, unactivated lifecycle whose `reminder_next_attempt_at` is still `NULL` (never special-cased by id) → expiry CAS transitions (deadline-passed rows win over reminders) → due-reminder processing → expiry side-effect reconciliation (note/Slack/founder-email, each independently retried via its own completion marker). Batches are capped at 25 rows per category, ordered by due time — no unbounded pagination.
- **Reminder send sequence** per candidate: `selectCatchUpStage()` picks the single latest overdue stage → fresh activation check → origin/venue resolution → an exact-match (never `<`) CAS lease claim pinning lifecycle id, prior `reminder_stage`, and lease/expiry/release state → a full re-validation of every pinned field immediately before sending (abandons only this worker's own lease on any mismatch, e.g. a concurrent extension) → send → **the `reminder_sent` Internal Note is written before the stage is ever acknowledged** (deterministic `event_key` = `hhc-activation-reminder:<lifecycleId>:<stage>`, so a duplicate insert is a safe no-op, not an error) → a second exact-match CAS advances `reminder_stage`, clears the lease, and schedules the next stage's due time. A zero-row match on that final CAS (email sent and noted, but the acknowledgement didn't land) raises a `critical` Slack alert (`#ops-critical`) for manual review rather than silently retrying with a fresh key.
- **Failure/retry:** up to 3 attempts per selected stage with ~1 hour backoff; after exhaustion the stage is left unresolved (never falsely marked delivered) and the schedule advances to the next stage's own natural due time — `selectCatchUpStage()`'s "latest overdue stage wins" behavior is what naturally supersedes an exhausted stage once a later one succeeds, with no extra persisted state needed.
- **Expiry:** a one-shot CAS sets `expired_at` once `deadline_at` has passed (never sets `released_at`); its three side effects (Internal Note, `#ops-alerts` Slack, founder email to `hello@happyhourcompass.com`) are each independently idempotent/retried via their own marker (`event_key`, `expiry_slack_notified_at`, `expiry_founder_email_sent_at`) and each re-checks operator activation immediately before sending.
- **Two separate exported functions, not one function with a `dryRun` flag.** `processActivationReminders()` (the LIVE entry, the only one the cron route imports) checks the kill switch first and has no `dryRun` field on its deps type at all — a caller literally cannot request planning mode through it. `planActivationReminders()` is the dedicated read-only planning entry and deliberately never checks the kill switch, so a preview stays usable even where `OPERATOR_ACTIVATION_REMINDERS_ENABLED` is unset (e.g. local dev) — its safety comes entirely from the same internal `if (!dryRun)` guards around every mutating/external call, permanently forced on. This split makes "a live caller can never reach planning mode, and vice versa" a type-level guarantee rather than a convention.
- **Dry-run:** `npm run activation-reminders:dry-run -- --dry-run` (`scripts/processActivationReminders.ts`, which imports only `planActivationReminders()` — never the live function) is the only sanctioned way to invoke this outside a test — it refuses to run without the literal `--dry-run` flag and has no `--apply`/live-mode flag at all. Dry-run reports every decision it would have made (`plannedActions`) without touching the database or sending email/Slack; it never recovers a stale lease.
- **Cron:** `src/app/api/cron/operator-activation-reminders/route.ts`, hourly (`vercel.json`), `CRON_SECRET` bearer auth matching the existing Customer Success route exactly, `maxDuration = 60`, no request-controlled parameters (no `dryRun`, no query/body) — the network surface can only ever trigger a real, non-dry-run pass, gated entirely by the kill switch.

### Operator activation lifecycle — Control Panel visibility & manual Release (Phase 2A-4 — 2026-09)

**This phase itself did not enable the reminder worker** — it has since been enabled in Production (see the Phase 2A-3 entry above). This phase is Control Panel visibility (surfacing the migration-099 worker fields that already existed but were invisible) plus one new founder action (manual Release) — it does not enable, deploy, or otherwise activate the worker.

- **Reminder/expiry fields are now visible.** `ActivationLifecycleSummary` (`src/lib/activation/activationPresentation.ts`) forwards all 9 migration-099 columns (`reminderNextAttemptAt`, `reminderAttemptCount`, `reminderLastAttemptedAt`, `reminderLastError`, `reminderLeaseStage`, `reminderLeaseStartedAt`, `expirySlackNotifiedAt`, `expiryFounderEmailSentAt`, plus the pre-existing `reminderStage`) — previously fetched via `.select("*")` but never mapped through. `ActivationCard`'s "Reminders" row is no longer a hardcoded "No automated reminders sent" string: it shows real stage progress, next scheduled attempt, a "Reminder currently processing" state when the worker's lease is held, the last failure/attempt count only when `reminder_attempt_count > 0`, expiry Slack/founder-email completion status for expired records, and a clear "Automated reminders are currently disabled" line whenever `isOperatorActivationReminderProcessingEnabled()` is false (resolved server-side and passed down as a plain boolean prop — the env var itself is never exposed to the browser).
- **Manual Release** (`releaseActivationLifecycleAction` → `activationReleaseImpl.ts`, same thin-`"use server"`-wrapper/no-DI-in-the-wrapper architecture as Extend/legacy-resume) closes one stalled activation lifecycle **and** returns its one originating venue to an unclaimed state — option B from the Phase 2A-4 audit, not lifecycle-only: leaving `created_by_operator_id`/`claimed_by` set would permanently lock the venue to an operator who never activated, since `resolveExistingVenueMatchAction` already refuses to re-approve a venue whose ownership fields are set.
  - **Exact mutations, nothing else:** lifecycle — sets only `released_at` (never touches `expired_at`, never creates/modifies any other lifecycle). Venue (the one resolved from this origin only) — clears `claimed_by`, `claimed_at`, `created_by_operator_id`; **`is_verified` and `is_published` are preserved exactly as they were** (no verification/publish-state decision was made in this phase); venue content is never touched. Claim/submission status/history, the operator record, the Supabase Auth user, `operator_memberships`, every other venue the same operator owns, subscriptions/customer records, and every existing Internal Note are all preserved.
  - **One-origin/one-venue only, never a bulk operator sweep.** The venue update is scoped by venue id and additionally requires `created_by_operator_id = lifecycle.operator_id` — never `.eq("created_by_operator_id", operatorId)` across all of that operator's venues. Because at most one *live* lifecycle can ever exist per operator (migration 098), a second approved-but-unactivated origin for the same never-activated operator has no lifecycle of its own to release through; after this lifecycle is released, that second origin is a genuine `not_tracked` record and may independently use the existing controlled legacy-activation-resume action to start its own fresh lifecycle later, on its own timeline. Release never automatically follows a phantom operator to their other venues.
  - **Eligibility, re-checked fresh at submit time, never trusted from page render:** founder authorized via `isControlPanelAdmin()` before any lookup; operator still unactivated (checked twice — once during eligibility resolution, once immediately before the lifecycle CAS); lifecycle unreleased; state is `release_required` or `expired` (never a still-live window); no reminder lease held; origin resolves unambiguously to exactly one venue; that venue's `created_by_operator_id` still equals the lifecycle's operator.
  - **CAS, not a transaction** — the lifecycle and venue writes are two separate network round trips (this codebase's Supabase client has no ambient cross-table transaction, and no RPC/migration was introduced to add one). The lifecycle CAS pins `id`, `released_at IS NULL`, `reminder_lease_started_at IS NULL` (same rule/message as Extend), exact `deadline_at`, exact `expired_at` (`.eq()`/`.is(null)`), and exact `reminder_stage`. On lifecycle-CAS failure: no venue change, no note.
  - **Partial-write reconciliation** (`reconcileVenueReleaseFailure()`) — on lifecycle success but venue-CAS failure (0 rows or an error), a fresh read of the one resolved venue decides the outcome, never a blind retry or a full transactional rollback claim: (1) all three ownership fields already null (a benign race — e.g. an earlier retry's write actually landed) → treated as complete, proceeds to the note step, no alert; (2) still owned by the same operator → **activation-safe compensation** (see below); (3) owned by a DIFFERENT operator or the linkage otherwise changed → that venue is never touched and the lifecycle is never blindly reopened, one `critical` alert, clear manual-review message. On success but the note insert failing: the release still stands (never re-run the venue mutation to recreate a note), a non-critical alert covers the missing audit trail.
  - **Activation-safe compensation (branch 2)** — since an operator could activate in the narrow window between this action's own eligibility check and this recovery attempt, `operators.account_activated_at` is re-checked BOTH before and after the compensating attempt, never just once: activated before compensation → never reopen, `released_at` stays exactly as this action left it; still unactivated → one guarded compensating rollback (`released_at → null`), CAS-pinned on the lifecycle id, the exact `released_at` value this call itself wrote, the exact prior `deadline_at`/`expired_at`/`reminder_stage`, and `reminder_lease_started_at IS NULL`; compensation succeeds and the operator is STILL unactivated → safe, retryable; compensation succeeds but activation occurred during that very gap → the lifecycle is immediately re-closed with an equally guarded CAS (never left live for an already-activated operator) and the outcome (re-close succeeded or failed) is escalated. **Exactly ONE alert covers the entire outcome of this branch**, however many of these steps a given call needs — never one alert before compensation and a second one after — stating the lifecycle/origin/venue ids and whether compensation was attempted, whether it succeeded, whether activation was detected before or after it, and whether a re-close was needed and succeeded.
  - **Alert delivery is dependency-injectable** (`ReleaseActivationLifecycleDeps.sendAlert`, defaults to the real `sendSlackAlert`) specifically so no behavioral test ever depends on Slack webhook environment variables being absent — every release test injects an explicit stub/spy. The thin `"use server"` wrapper exposes no `deps` parameter of any kind (proven by a dedicated source-wiring test), so this seam carries zero production risk.
  - **Structured note:** `founder_manual_release` (already-reserved event type, label "Venue released by founder"), deterministic `event_key` = `hhc-activation-release:<lifecycleId>` (a duplicate insert is treated as already-recorded, not an error), founder-attributed (real signed-in founder, never the system author — `writeActivationNote()` can't be reused here since it hardcodes system attribution, so this is a direct insert), metadata `{lifecycleId, venueId, previousDeadline, releasedAt, flow}`, written only after both writes succeed.
  - **Notifications:** Release sends **no** operator email and **no** additional founder Slack/email — the automated expiry pipeline already delivered both when the lifecycle first expired; a second notification would be redundant. **Notification incompleteness never blocks Release** — a founder can release an expired lifecycle even if `expiry_slack_notified_at`/`expiry_founder_email_sent_at` are still null (though note that once released, `processActivationReminders.ts`'s own reconciliation query — `released_at IS NULL` — will no longer retry those two side effects for this lifecycle; this is a known, accepted tradeoff, not a bug).
- **Action Center card — "Operator activation reviews"** (`src/lib/activation/activationReviews.ts`, wired into `src/lib/data/actionCenter.ts` and a new report at `/control-panel/action-center/reports/operator-activation-reviews`). Queries `operator_activation_lifecycles` directly (`released_at IS NULL`) as the starting point — never every Claim/Submission in the system — then batch-resolves operators/origins/venues in a small, fixed number of `.in(...)` queries regardless of row count, following the same N+1-avoidance discipline as `activationPresentation.ts`. Counts the de-duplicated union of lifecycles matching any of: `release_required`, `expired` (unreleased), `reminder_exhausted`, or `expired` with an incomplete Slack/founder-email notification — each lifecycle counted once in the primary total even when it matches multiple reasons. Activated operators and released/ordinary awaiting-setup/expiring-soon lifecycles are never included.
  - **`reminder_exhausted` exact predicate** (corrected — see `computeReasons()`'s own header comment): not expired, deadline still in the future, `reminder_stage < 3`, `reminder_next_attempt_at IS NULL`, a real failure signal (`reminder_last_attempted_at` and/or `reminder_last_error` non-null), and an attempt-count signal consistent with exhaustion. That last check deliberately accepts `reminder_attempt_count >= 3` **or** `=== 0` — `processActivationReminders.ts`'s `handleReminderFailure()` resets the counter to 0 the moment a stage is exhausted (a fresh stage gets a fresh budget), so the real, persisted shape of a genuinely exhausted lifecycle has `reminder_attempt_count === 0`, not 3. A brand-new, never-touched lifecycle (Kelly's current live row: `reminder_stage 0`, `reminder_next_attempt_at null`, `reminder_attempt_count 0`, no last attempt/error) is still correctly excluded — the mandatory failure-signal requirement (`reminder_last_attempted_at`/`reminder_last_error`) is what discriminates it from a genuinely exhausted row, not the attempt count alone.
- **List filters** on both Claims and Submissions gained three previously-missing activation options — Expired, Released, Not tracked — alongside the existing four; the attention-first default sort (`attentionRank()`) is completely unchanged, so active/released rows still never become attention items.
- **Internal Note metadata allowlist** (`src/components/ActivationNoteMeta.tsx`) extended to render the Phase 2A-3 worker's own note fields (`stage`, `deadline`, `sentAt`, `attempt`, `flow`, `lifecycleId`, `operatorEmail`) and this phase's `venueId`/`releasedAt` — previously these notes rendered a friendly label with zero metadata detail. Still strictly allowlist-only; an unrecognized key stays hidden.

### Operator email-code activation (email-code initiative Phase 1B foundation — 2026-09)

A separate initiative from the Control Panel "Phase 1B" above — don't conflate the two. **Foundation only: the schema is live in Production, but no user-facing behavior exists — the feature is not enabled anywhere.** Approved design is **Model A**: approval → provision operator/auth identity **and** activation lifecycle (unchanged — so the existing 14-day window and reminders still recover an operator who abandons before code verification, after verification but before password creation, or later) → verify email with a six-digit code → create password → activate/sign in → existing Operator Dashboard.

- Migration `100_operator_email_code_verification_foundation.sql` is **applied to Production** (2026-09-25; commit `8c5fc38`). Its own header still reads "STATUS: not yet applied" — leave it: the applied file must stay byte-for-byte unchanged so its checksum stays stable. It adds `verification_required` (default `false`) / `verification_completed_at` to `operator_activation_lifecycles` and a new internal-only `operator_verification_codes` table (HMAC digest only, never plaintext; at most one current and at most one consumed code per lifecycle via partial unique indexes).
- **All code writes go through three `service_role`-only `SECURITY DEFINER` functions** (`SET search_path = ''`), each locking the lifecycle row first: `issue_operator_verification_code()` (eligibility, 60-second cooldown, 5-per-rolling-60-minutes cap, supersede + insert in one transaction — only an `issued` outcome may be followed by an email), `record_operator_verification_code_failure()`, and `consume_operator_verification_code()`. `service_role` has SELECT-only on the table. `attempt_count` counts **incorrect** attempts only; a correct code never increments it, and after 5 incorrect attempts even the correct code is rejected. Per-IP/per-email limits are **not** implemented yet. See the migration header for the full design.
- **Legacy-by-default rule:** `verification_required = false` means the legacy setup-link flow. Every existing lifecycle is legacy, and no existing lifecycle is ever opted in; only a future, separately authorized approval path may create a lifecycle with `verification_required = true`.
- Feature flag `OPERATOR_EMAIL_CODE_VERIFICATION_ENABLED` and HMAC secret `OPERATOR_VERIFICATION_CODE_HMAC_SECRET` (≥32 chars, distinct per environment) are **not set in any environment** — readers in `src/lib/activation/emailCodeVerificationConfig.ts`; pure policy/security functions in `emailCodeVerificationPolicy.ts`; types in `emailCodeVerificationTypes.ts`. None of these are imported by any flow (pinned by `tests/unit/activation/emailCodeVerificationWiring.test.ts`).

### Claim auto-approval (2026-09)

Normal claims auto-approve and the claimant stays in the HHC flow (claim → `/operator/verify` with the first code already sent → Set Password → Operator Admin). Founder review is the exception. Flag `CLAIM_AUTO_APPROVAL_ENABLED` (fail closed) — **absent/off ⇒ `submitClaimAction` behaves exactly as before** (no evaluation, no new notes, original notifications). Auto-approval also requires the email-code flow to be available for the claimant (it never auto-approves onto the legacy setup-link path).

- **Modules:** `src/lib/claims/` — `claimAutoApprovalPolicy.ts` (pure decision engine), `claimAutoApprovalSignals.ts` (read-only signal gathering), `claimAutoApprovalFlow.ts` (orchestration + retry continuation), `claimAutoApprovalNotifications.ts`, `claimAutoApprovalNotes.ts`, `claimContinuation.ts` (client-safe redirect allowlist).
- **Rules (not a points score):** any hard condition H1–H7 → founder review; else a strong positive (business-domain match, last-10-digit phone match, activated operator) → auto-approve; else foreign IP + another caution, or 3+ of the other cautions → review; else auto-approve. H7 ("auto-approval unavailable") is technical routing and must never be described as claimant risk. Gmail, phone mismatch, remote domestic IP, unresolved geo, and missing data are neutral on their own.
- **Geo:** Vercel edge headers (`x-vercel-ip-*`), trusted only when `VERCEL=1`; unknown ⇒ neutral. `ip-api.com` is display-only (Control Panel) and must not become a decision dependency.
- **Ownership:** the claim is inserted `pending` first (the one-pending-per-venue index serializes competitors), CAS'd to `approved` (`reviewed_by` null = auto), then provisioned with `requireUnclaimedVenue` (conditional link — never overwrites a concurrent owner; a lost race rolls back and returns the claim to `pending` with a founder notification). Auto-approved claims for a not-yet-activated operator pass `markVenueVerified: false`; `completeOperatorAccountActivation()` sets `is_verified` when activation completes. Both options default to the old behaviour for every other caller.
- **Visibility:** claim notes (which the venue page already merges into Venue Internal Notes) record submitted → `auto_decision` (readable reasons + structured metadata, event key `hhc-claim-auto-decision:<claimId>`) → activation events. Founder email + `#venue-claims` Slack for every claim: "CLAIM AUTO-APPROVED" or "manual review required because…" in plain English.
- **Not in this phase:** automatic release of expired auto-approved claims (existing 14-day window, reminders, expiry alert, manual Release apply).

### Consumer signup notifications

On successful consumer account creation (`createConsumerAccount()`, `operator-admin/src/app/(consumer-auth)/sign-up/actions.ts`):
- Consumer receives the branded confirmation email (`sendConsumerSignupConfirmationEmail`).
- Slack notification is sent to `#consumer-signup`.
- Founder notification email is sent to `hello@happyhourcompass.com` (`sendConsumerSignupFounderNotificationEmail`).

**Rule:** Notification failures (email or Slack) must never block successful account creation — each is fire-and-forget with its own error logging, matching the founder-notification pattern already used for claims/suggestions/submissions in `src/lib/email.ts`.

---

## Bot Protection (Cloudflare Turnstile)

Every unauthenticated public form/server action that writes to the database, creates an account, sends an email, or sends a Slack notification is gated by Cloudflare Turnstile. Client-side widget completion is never sufficient on its own — every protected server action re-verifies the token server-side via Siteverify before doing anything else.

### Architecture

- `operator-admin/src/lib/turnstile.ts` — server-only. `verifyTurnstileToken(token, remoteIp?)` calls Cloudflare Siteverify and returns `{ success: true }` or `{ success: false, reason }`. A missing token, a failed/expired/already-used token, and a Siteverify network failure are all treated identically: verification did not succeed. Also exports `TURNSTILE_FAILURE_MESSAGE` (the shared user-facing error copy) and `TURNSTILE_TOKEN_FIELD` (the FormData field name, `cf_turnstile_token`, used by every form-action-based flow).
- `operator-admin/src/components/Turnstile.tsx` — the one shared client widget. Renders Cloudflare's official script (`https://challenges.cloudflare.com/turnstile/v0/api.js`) via `next/script` and explicit `window.turnstile.render()` (not the implicit `data-sitekey` div), so a parent form can reset the widget imperatively through a forwarded ref (`TurnstileHandle.reset()`) after a failed submission. `onVerify(token)` fires on completion; `onExpire()` fires on token expiry, widget error, or timeout — callers must clear any stored token in response.
- No third-party Turnstile wrapper package is used — Cloudflare's script + Siteverify's plain HTTP API cover every case here.

### Protected flows

| Flow | Server action | Client form(s) |
|---|---|---|
| Contact Us | `submitContactAction` (`(consumer)/contact/actions.ts`) | `ContactForm.tsx`, website `ContactUsModalContent.tsx` |
| Suggest a Venue | `submitSuggestionAction` (`(consumer)/suggest/customer/actions.ts`) | `SuggestionForm.tsx`, website `SuggestVenueModalContent.tsx` |
| Add Your Venue | `saveOperatorSubmissionAction` (`(consumer)/suggest/owner/actions.ts`) | `OwnerSubmissionFlow.tsx`, website `AddVenueModalContent.tsx` |
| Add Venue — more info follow-up | `submitMoreInfoAction` (`(standalone)/suggest/owner/more-info/[token]/actions.ts`) | `MoreInfoForm.tsx` |
| Claim Your Venue | `submitClaimAction` (`(consumer)/venue/[id]/claim/actions.ts`) | `ClaimForm.tsx`, website `ClaimVenueModalContent.tsx` |
| Claim — more info follow-up | `submitClaimMoreInfoAction` (`(standalone)/claim/more-info/[token]/actions.ts`) | `MoreInfoForm.tsx` |
| Consumer signup | `createConsumerAccount` (`(consumer-auth)/sign-up/actions.ts`) | `sign-up/page.tsx` |
| Operator forgot password | `forgotPasswordAction` (`app/forgot-password/actions.ts`) | `ForgotPasswordForm.tsx` |
| Consumer forgot password | `requestConsumerPasswordReset` (`(consumer-auth)/account/forgot-password/actions.ts`) | `account/forgot-password/page.tsx` |

The consumer forgot-password flow previously called `supabase.auth.resetPasswordForEmail()` directly from the browser with no server action at all — there was no point to gate. It was moved into a small server action for exactly this reason; behavior (redirect target, always-return-success anti-enumeration) is unchanged, just moved server-side. It has since been changed again to call `admin.generateLink()` + send its own Resend email instead of calling `resetPasswordForEmail()` at all — see the "Supabase Auth email delivery (SMTP)" section above; the Turnstile gate and anti-enumeration behavior are unaffected by that change.

The Add Your Venue flow is multi-step (business lookup → match confirmation / no-match / rejection). `lookupBusinessAction` (the Google Places lookup) is **not** Turnstile-gated — it performs no database write, account creation, email, or Slack notification. Verification instead happens inside `saveOperatorSubmissionAction`, the actual side-effecting call, which is reached from all three terminal steps (confirm match, reject match, no-match continue); the widget is rendered on each of those three steps, immediately before its submit action.

**Not protected, intentionally:** operator/founder authenticated actions (Operator Admin, Founder Control Panel), read-only browsing/search/saved-items interactions, and `src/app/api/track/*` analytics endpoints (unauthenticated but not a submission/lead-capture surface).

### Server-side verification rule

Every protected server action follows the same shape:
1. Run existing field validation first (cheap, no network) — return field errors before touching Turnstile.
2. Extract the token from `formData.get(TURNSTILE_TOKEN_FIELD)` (form-action flows) or an explicit `turnstileToken` parameter (flows that call the server action directly, e.g. `createConsumerAccount`, `saveOperatorSubmissionAction`).
3. Call `verifyTurnstileToken()`. On failure, return immediately with the shared failure message and a `turnstileFailed: true` flag on the result — **before** any database write, `auth.admin.createUser`/`generateLink`, email send, or Slack notification.
4. Only proceed to the flow's existing side effects once verification succeeds. Existing behavior (emails, Slack notifications, redirects, success confirmations, the "notification failures never block a valid submission" rule) is otherwise unchanged.

On the client, `turnstileFailed: true` in the result is what triggers `turnstileRef.current?.reset()` and clears the stored token — an ordinary validation error does not reset the widget, since the token is still valid for the next attempt.

### Environment variables

Uses the existing `NEXT_PUBLIC_TURNSTILE_SITE_KEY` (client-side, safe to expose) and `TURNSTILE_SECRET_KEY` (server-only — never import `src/lib/turnstile.ts` from a Client Component). Both already exist locally and in Vercel Preview/Production; no new environment variables were introduced.

### Content Security Policy

This codebase has no CSP anywhere (`next.config.ts` has no `headers()` block; `middleware.ts` sets no security headers). Turnstile therefore required no CSP update — there is nothing to conflict with. If a CSP is introduced later, it must allow `https://challenges.cloudflare.com` for `script-src`, `frame-src`, and `connect-src`.

---

## Supabase Access (MCP & CLI)

### Preferred method: Supabase MCP server

- Configured in `.mcp.json` (repo root, committed): `@supabase/mcp-server-supabase`, pinned to `--project-ref=juphyhxdmcvseeufbiay`, launched with `--read-only`.
- Authenticates via `SUPABASE_ACCESS_TOKEN`, supplied through `.claude/settings.local.json` (gitignored — see `docs/developer/SETUP.md`). This is a Management API personal access token, not a database password or service-role key.
- **This server is read-only by explicit configuration**, independent of the token's own scope. Use it for schema inspection (`list_tables`, `list_migrations`, `list_extensions`), security/performance review (`get_advisors`), read-only SQL (`execute_sql` — `SELECT`/`information_schema` queries only), and project metadata (`get_project_url`, `get_publishable_keys`).
- **Confirm you're targeting the correct project** by calling `get_project_url` (needs no token) and checking the returned host is `juphyhxdmcvseeufbiay.supabase.co` — the one project behind local dev, staging, and production (see the Repository layout note above).

### Fallback: Supabase CLI

- The `supabase` CLI holds its own separately authenticated session (not the MCP token) and is already linked to this project. `supabase projects list` confirms this — it should show `juphyhxdmcvseeufbiay` / `happy-hour-compass` with `"linked": true`.
- Useful as a fallback for project/schema metadata when the MCP server is unreachable.
- `supabase status` targets the **local** Docker-based dev stack, not the remote project, and fails with a Docker-daemon error whenever Docker isn't running locally. That failure says nothing about remote Supabase access — don't read it as the CLI's authenticated session being broken.
- A CLI write/migration path against the remote project has not been established as authorized in any Claude session to date — don't assume `supabase db push` or similar is available here without confirming with Wayne first.

### Writes, migrations, and configuration changes

- Migration files live in `supabase/migrations/` (see **Supabase migrations** below for authoring rules). How they get applied to the live project is separate from how Claude *reads* it, and has not been verified in any Claude session — confirm the authorized apply path with Wayne before assuming one.
- The MCP server's `--read-only` flag means it cannot perform writes or run migrations regardless of the token's own permissions — treat this as a deliberate safety boundary, not something to route around.
- Any database write, migration, Auth configuration change, or other Supabase project-configuration change requires explicit authorization from the current task. A working read-only connection is never itself authorization to write.

### If Supabase MCP returns `Unauthorized`

Do not treat the first `Unauthorized` response as proof Claude lacks Supabase access. Work through this sequence in order before concluding otherwise:

1. Confirm the intended MCP configuration is actually loaded — check `.mcp.json` and that `.claude/settings.local.json` has `SUPABASE_ACCESS_TOKEN` set (presence only; never print, compare, or log its value).
2. Check whether the *running* MCP server process is holding a stale token or configuration from before a credential was last updated. **A token file can hold the current credential while an already-running MCP process still holds an older one from when it launched** — this is a confirmed failure mode in this project, not a hypothetical.
3. Reconnect or restart the Supabase MCP server so it reloads current configuration. If that isn't possible from within the session, tell Wayne the Claude session needs to be restarted.
4. Retry the same read-only MCP calls.
5. If still failing, check the Supabase CLI fallback (`supabase projects list`) as an independently authenticated path — this distinguishes a stale-MCP-process problem from an actual credential or project problem.
6. Only report Supabase as unavailable after steps 1–5 have all been checked and have failed. `Unauthorized` on a first call is never sufficient grounds on its own to report unavailability.

### Security & authorization boundaries

- Never print, expose, copy, commit, or document access tokens, passwords, service-role keys, database passwords, or other secret values — including in this file. Diagnose token problems by checking *presence*, and at most length, never by displaying or comparing values.
- Never invent credential workarounds. Never regenerate, replace, revoke, or rotate any credential without Wayne's explicit approval. Never transfer a credential into another tool, file, or location without explicit approval.
- Read-only access does not authorize writes. Database writes, migrations, Auth/config changes, and other production configuration changes must be explicitly authorized by the current task, not inferred from having a working connection.
- Use the least-privileged established method that can complete the authorized work — prefer MCP read-only tools over the CLI, and never reach for a write-capable path just because a read-only one is inconvenient.
- An access failure that survives the full troubleshooting sequence above is a stop-and-report condition — report it to Wayne rather than working around it.

---

## Supabase migrations

### Every new public-schema table must have explicit GRANTs

Supabase removes automatic grants for `anon`, `authenticated`, and `service_role` on new public-schema tables starting **October 30 2026**. Any table created without explicit GRANTs will be silently inaccessible via the Supabase Data API.

**Rule:** Every migration that creates a `CREATE TABLE` in the `public` schema must end with a GRANT block.

**Template:** Copy `supabase/migrations/_template.sql` — the GRANT section is pre-filled with the correct structure and inline guidance.

**Reference:** `supabase/migrations/039_security_hardening.sql` documents the full grant philosophy:
- `anon` — public-facing intake forms only (no login gate required)
- `authenticated` — operator-facing tables accessed directly by the app
- `service_role` — all tables, always (bypasses RLS; required for `createAdminClient()`)
- Scope each role to the minimum operations it actually performs (`SELECT`, `INSERT`, `ALL`, etc.)
- Never grant `ALL` to `anon` or `authenticated`

### RLS must be enabled on every new table

Always include `ALTER TABLE public.<table> ENABLE ROW LEVEL SECURITY;` even if no permissive policies are added yet. No permissive policy + RLS enabled = inaccessible to all non-service-role callers by default (safe baseline).

---

## Seeded Market Launch Prep — Google Places Refresh Tool

When preparing any previously-seeded market for activation/launch (Central Okanagan was the pilot; this applies equally to Greater Vancouver, Seattle, Toronto, or any future market), run the **Seeded Venue Refresh** tool before the separate manual Happy Hour certification pass:

```
npm run refresh:seeded-venues -- --market=<market-slug> --dry-run
npm run refresh:seeded-venues -- --market=<market-slug> --apply   # only after reviewing the dry run
```

`<market-slug>` is any `markets.slug` already in the database (e.g. `central-okanagan`, `greater-vancouver`) — run with no `--market` to print the valid list. **Dry run is the default and is mandatory before apply** — a normal invocation makes zero database writes; only `--apply` writes anything, and only after the dry-run output has been reviewed.

**What it does:** `operator-admin/scripts/refreshSeededVenues.ts` queries the Google Places API (New) for every venue in the selected market and compares Google's current data against the production row — `place_id`, Google operating status, `business_hours`, `phone`, `website_url`, `google_rating`, `google_review_count`, latitude/longitude, address/postal code, and `google_primary_type`/`google_types`. It reuses the existing proven Get-Place/Text-Search-fallback matching logic (place_id first, then name-similarity + geographic-distance-guarded text search) already used by `scripts/backfillMissingBusinessHours.ts`, `scripts/backfillGoogleRating.ts`, and `searchGooglePlace()` in the operator-submission flow — it does not reinvent that matching algorithm.

**What it explicitly does NOT do:**
- **Happy Hour data is out of scope.** `hh_times`, `hh_food_details`, `hh_drink_details`, and `hh_tagline` are never read for comparison and never written by this tool — Google Places has no concept of "happy hour." Happy Hour data still requires the separate manual Happy Hour certification workflow (see the Data Hygiene process used for the BTS Cocktail Bar & Kitchen pilot). The tool's CSV output exists to feed that workflow, not replace it.
- **`establishment_type` is never auto-updated.** It's a curated Happy Hour Compass product field, not a mirror of Google's `primaryType`/`types`. A mismatch is only ever surfaced as a review flag for a human to judge.
- **Geography is never auto-updated.** Latitude/longitude, address, and postal-code differences are always flag-only — a geographic mismatch can mean a bad Google match as easily as a stale DB value.
- Never touches `is_published`, `claimed_at`, `claimed_by`, `created_by_operator_id`, `updated_by_operator_id`, `source`, `placeholder_image_path`, media, editorial copy, payment/plan data, guides, or events. `--apply` only ever writes to venues where `source = 'seed' AND claimed_at IS NULL AND created_by_operator_id IS NULL` — once an operator can edit a venue from Operator Admin, this tool must never silently overwrite their data with Google's.
- Google `CLOSED_TEMPORARILY` / `CLOSED_PERMANENTLY` is never auto-actioned (no auto-unpublish, no delete) — only a prominent review flag.

**Output:** a console dry-run report (per-venue match method/confidence and field-by-field current-vs-Google-vs-proposed, plus a summary with proposed-change counts by field and a manual-review list), and a CSV **"Happy Hour review worksheet"** under `operator-admin/scripts/output/` (gitignored — production data snapshots are never committed). This CSV is the permanent working document for the manual Happy Hour review step of every market launch (run refresh → generate CSV → manual HH review, currently Wayne + ChatGPT → HH import → QA) — not a one-off report. One row per venue, columns grouped to follow that review workflow rather than the database schema: Identification → Google verification → Venue verification (current vs. Google phone/website/business hours, each with a status, plus establishment_type vs. Google's primary type as a flag) → Happy Hour **current** data in human-readable columns (`current_hh_schedule`, `current_food_1_name`/`_price`/`_notes` ×3, `current_drink_1_name`/`_price`/`_notes` ×3 — read-only, never JSON, never compared or modified by this tool) → Happy Hour **new** columns (`new_hh_schedule` as a single field, then `new_food_1_name`/`_price`/`_notes` ×3 and `new_drink_1_name`/`_price`/`_notes` ×3 — structured to exactly mirror the `current_*` item columns rather than free text, always blank on generation, filled in by hand during manual review — so a future HH import tool can map them deterministically into `hh_food_details`/`hh_drink_details` without parsing reviewer prose) → workflow (`manual_notes`, pre-seeded with this run's system review flags; `workflow_status`, blank by default — set by hand to e.g. "Not Started"/"In Review"/"Complete") → a trailing reference group of secondary system columns. Re-running the tool always regenerates a fresh snapshot — it does not read back or merge a previously hand-edited CSV, so save in-progress manual review work before re-running for the same market.
