...
Claims (`venue_claims`) and Add Your Venue submissions (`operator_submissions`) share one operator-activation lifecycle: both provision an operator via the same `provisionOperatorForVenue()` and wait for `operators.account_activated_at` to be set. Migration `098_operator_activation_lifecycle_foundation.sql` (**not yet applied**) adds the schema this lifecycle needs — **foundation only; the rules below describe what exists today, not the full planned system**:
...
- `src/lib/activation/activationState.ts`'s `deriveActivationState()` is the single place lifecycle state (Awaiting Setup / Expiring Soon / Active / Expired / Released / Not Tracked) is derived — always from `operators.account_activated_at` plus a lifecycle row's own columns, never from whether an email/code was sent. No second stored "status" column exists or should be added.
...
**Not yet implemented (do not assume otherwise):** activation reminder emails, a reminder cron, expiry/release execution, Control Panel visibility for this lifecycle, the six-digit OTP verification flow, and auto-approval changes to claim/submission routing. See the Phase 1A task's own final report for the full boundary and the follow-on phases this unblocks.
