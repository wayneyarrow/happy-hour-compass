...
- **N+1-safe data access** — `src/lib/activation/activationPresentation.ts` (server-only, admin-client) batches Claim/Submission list pages into exactly 2 queries total (one lifecycle lookup via `.in(origin_..._id, ids)`, one deduplicated operator lookup), regardless of row count. Single-item wrappers (`getActivationPresentationForClaim`/`ForSubmission`) reuse the same batch path for detail pages. Never creates a lifecycle merely by being read. Not imported by any public/consumer/website route.
