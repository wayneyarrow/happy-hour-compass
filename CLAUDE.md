...
Only **one** flow in the app currently relies on Supabase Auth's own mail delivery: consumer forgot-password (`requestConsumerPasswordReset`, `operator-admin/src/app/(consumer-auth)/account/forgot-password/actions.ts`), which calls `supabase.auth.resetPasswordForEmail()` and lets Supabase send the recovery email itself. Every other Auth-adjacent email — consumer signup confirmation, operator activation, operator forgot-password — calls `supabase.auth.admin.generateLink()` to obtain the link only, then sends its own branded email via Resend (`src/lib/email.ts`), deliberately bypassing Supabase's mailer.

**Rule:** Supabase Auth must have **Custom SMTP enabled** (Supabase dashboard → Authentication → Emails → SMTP Settings), routed through the project's existing Resend account, using the domain already verified there (`happyhourcompass.com` — SPF/DKIM confirmed via Resend + DNS). Without it, Supabase falls back to its own default/shared mail infrastructure, which is not intended for production use (low fixed send-rate limits).
...
- The Supabase **Reset Password email template** must build its link from `{{ .RedirectTo }}?token_hash={{ .TokenHash }}&type=recovery` instead of `{{ .ConfirmationURL }}` — this keeps the link on our own domain (`{{ .RedirectTo }}` reflects the per-request `redirectTo` our code already passes via `getSiteUrl()`) instead of the raw Supabase project domain. Do **not** use `{{ .SiteURL }}` for this — it's a single dashboard-configured value that doesn't vary per environment (see "Environment-aware authentication links" above), unlike `{{ .RedirectTo }}`.
