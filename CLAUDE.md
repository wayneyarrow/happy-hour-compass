...
**Schema and pure-policy foundation.** Migration `099_operator_activation_reminders.sql` is applied. See the Phase 2A-3 entry below for the worker that now reads/writes this schema (still kill-switched off in production).
...
**The worker exists and is fully wired (cron route, `vercel.json` schedule, dry-run script) but is deliberately kept inert in production**: `isOperatorActivationReminderProcessingEnabled()` (`src/lib/activation/activationReminderConfig.ts`) checks `process.env.OPERATOR_ACTIVATION_REMINDERS_ENABLED === "true"` as the literal first line of `processActivationReminders()` — no such variable has been added to any Vercel environment, so every hourly cron invocation currently returns immediately with zero reads, zero writes, and zero external calls. Do not add the environment variable without separate explicit authorization.
...
- **Two separate exported functions, not one function with a `dryRun` flag.** `processActivationReminders()` (the LIVE entry, the only one the cron route imports) checks the kill switch first and has no `dryRun` field on its deps type at all — a caller literally cannot request planning mode through it. `planActivationReminders()` is the dedicated read-only planning entry and deliberately never checks the kill switch, so a preview stays usable while `OPERATOR_ACTIVATION_REMINDERS_ENABLED` remains unset (the normal state today) — its safety comes entirely from the same internal `if (!dryRun)` guards around every mutating/external call, permanently forced on. This split makes "a live caller can never reach planning mode, and vice versa" a type-level guarantee rather than a convention.
...
**Reminder automation is still fully disabled and undeployed to Production** — nothing in this phase changes that. `OPERATOR_ACTIVATION_REMINDERS_ENABLED` remains unset in every Vercel environment; the worker/cron from Phase 2A-3 still performs zero reads/writes/external calls. This phase is Control Panel visibility (surfacing the migration-099 worker fields that already existed but were invisible) plus one new founder action (manual Release) — it does not enable, deploy, or otherwise activate the worker.
