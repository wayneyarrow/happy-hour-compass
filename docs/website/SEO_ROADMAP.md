...
- **Private-page noindex handling** — `/account` and `/welcome` now carry the same `robots: { index: false }` pattern already used by `/saved` — all three are private, user-specific pages, correctly noindexed unconditionally in every environment.
- **Public discovery pages indexing fix (HHC SEO Indexing Audit, 2026-09-16)** — `/website-happy-hours`, `/website-daily-specials`, and `/website-events` were previously miscategorized alongside the private pages above and hardcoded an unconditional `robots: { index: false }`, which forced noindex in production as well as staging — confirmed by Google Search Console as the cause of these pages being unindexable live. These are primary consumer discovery pages, not private routes; they now use the same environment-aware `buildPageMetadata()`/`shouldNoIndex()` pattern as every other public page, and are included in `sitemap.ts`.
